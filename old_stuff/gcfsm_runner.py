...
class GCfsm (BaseWorker):
    ...
    @property
    def our_color(self):
        return Team.YELLOW if self.us_yellow is True else Team.BLUE        
    @property
    def team_info(self,team:Team):
        return self.yellow_info if team == self.our_color else self.blue_info
    ...
    def step(self):
        ...
        if self.last_ref_msg is not None:
            ...
            if new_ref_msg.packet_timestamp < self.last_ref_msg.packet_timestamp:
                return
        # updated last received to this new message
        

        self.last_ref_msg = new_ref_msg
        # print("[GCFSM] : STATE from game_controller is: ", self.shared_state.name)
    
    def check_for_updates(self,new_ref_msg:RefereeMessage):
...
# checks commands and updates state accordingly
    ...
    def check_game_events(self,new_ref_msg:RefereeMessage):
        ...
        our_team_info = self.team_info(self.our_color)
        location = None
        ...
        for e in game_events:
            ...
            if e.type == GameEventType.BOT_SUBSTITUTION : 
                print(e.event_data)
                if e.event_data.by_team == self.our_color:
                    if new_ref_msg.bot_substitutions_left != self.subs_left : 
                        self.logger.warning("we sub robot")
                        self.subs_left = new_ref_msg.bot_substitutions_left
